...
| --- | --- | --- |
| Chat contract | Gateway unit tests cover valid requests, malformed input, timeouts, inactive webhooks, invalid credentials, exhausted credit, provider network failure, rate limiting, malformed provider output, and secret suppression | Passing locally |
| Container health | Phase 7 smoke starts the isolated stack, checks both health endpoints, restarts containers, and checks recovery | Passing locally on 2026-07-26 |
| Occupied ports | Phase 7 smoke runs preflight from a second project while both configured ports are held by the test stack | Passing locally on 2026-07-26 |
| Pilot evidence | Unit tests prove a complete passing fixture returns `GO`, incomplete evidence returns `NO_GO`, and personal-data fields are rejected | Passing locally |
| Browser widths | Pinned Chromium checks at 375, 768, and 1440 pixels | Passing locally |
| Windows helpers | PowerShell files are parsed on a Windows GitHub Actions runner | Pending the first CI run |
