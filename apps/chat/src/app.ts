...
// A paid run carries rankings, keyword ideas, and SERP evidence, so its saved
// payload needs more room than ordinary business memory.
...
// The one gateway research workflows may use to read a business home page.
// It is HTTPS-only, resolves to public addresses only, and refuses to follow a
// redirect off the requested domain, so a workflow can never be steered at a
// private host.
...
function validateSeoSnapshot(body: unknown): SeoSnapshotInput {
  ...
  // The reviewed application ceilings: refresh US$0.10, standard US$0.20, deep US$0.50.
      ...
      if (url.pathname === "/api/public-domain-page") {
        try {
          ...
          if (error instanceof PublicError) {
            ...
            options.logError?.("Could not safely read the public domain", error);
            sendError(
              response,
              new PublicError(
                502,
