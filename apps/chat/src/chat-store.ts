...
export class ChatStore {
  ...
  private migrate(): void {
    ...
    if (version < 3) {
      // Paid domain research adds a historical SEO snapshot per attempt and
      // needs research jobs to be able to record an honest 'failed' outcome.
      // Existing conversations, messages, and business memory are untouched:
      // the job rows are copied across to widen one CHECK constraint.
      this.transaction(() => {
        this.database.exec(`
  ...
  // Every paid attempt is kept as history. Only a completed or partial run may
  // carry matching business memory forward; a failed run records its own state
  // and never replaces the last successful company memory.
